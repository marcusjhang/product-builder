...
- [ ] Tech-lead findings have no open "act on"; product-panel and persona findings have no open "act on", and at Feature or Program size both panels were cast (a Feature plan with either uncast is red; Bounded: "not cast, Bounded" is green); the rulings table has one row per finding the panels returned and its counts match the findings blocks; a high-importance assumption has a spike verdict or an accepted risk with a reason, and only a low-importance one may pass with an owner.

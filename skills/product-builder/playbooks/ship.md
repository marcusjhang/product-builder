...
2. Pre-flight: walk the Definition of Done items that precede the flip; a red item routes back to QA, Implement, or docs. Freshness: the review and QA verdicts must name the head SHA that is deployed; a verdict for an older head is stale and is re-run before the flip.

...
Open a todo list whose first items are the matched playbook's steps, copied in verbatim, before any task-specific todos. A step you choose not to do stays in the list with a one-line `skip: <reason>`. Rewrite the list at every gate close and at every playbook switch (plan to implement, implement to QA, a revise); the reply that closes a gate re-prints the changed rows. A list that has not changed in three turns while work moved is a bug, and the user should never have to ask what you are on. A reply that ends with a playbook step still unchecked, no question to the user, and no blocker named has stopped early: a leaf skill's reply is not the playbook's reply, so after a leaf skill returns, the next step runs, until every step is checked or carries `skip: <reason>` and the playbook's Reply contract is met. Match the task to a playbook below, open its file under `${CLAUDE_SKILL_DIR}/playbooks/`, and copy its steps.

Routing reads the state before the words. An argument that opens with a playbook name and a colon (`incident: checkout errors up 40x`, `flaky-test: the upload spec`) is matched without judgment. Otherwise, before matching, run `${CLAUDE_SKILL_DIR}/scripts/state` in one Bash call and keep its output for the first reply. It prints the profile's presence, every plan folder with its status line, the branch, ahead and behind the default branch after a fetch, uncommitted changes and the signals in them (a lockfile, a migration, a test), the commits and diff a branch is ahead by, and, through the profile's forge tool, the open PRs for the branch with their checks. You add the one fact a script cannot read: whether the pasted text or the last tool output holds a stack trace or a failing test. A state fact the script could not read (the forge tool absent, the fetch failed) is printed as such and is not "absent"; the route line quotes the facts that chose the route. Then match: a plan folder the words name or the branch belongs to routes by that folder's status (Ready → Implement, Built → QA, QA passed → Ship, anything else with a change → Revise); a branch ahead of the default with code and no plan folder, or a PR with no plan folder, routes to Built first when the words are about reviewing or showing the work ("look at this", "review what I built"); "open a PR" on the same state routes to Opening a PR, whose body names the missing plan and whose reply offers Built first as the next move rather than running the panels; an open PR with red checks or unanswered comments routes to Babysit; a stack trace or a failing test routes to Bug fix, or to Incident when the words or the state say production is affected now; a lockfile diff routes to Dependency upgrade and a migration file to Data migration; a `<slug>/p<n>-` branch with "continue" or no argument routes to Pickup and pause. The words decide when they name a playbook or one of its triggers; the state decides when the words are generic ("look at this", "done", "continue", "is this ok"); when the two disagree, one question with both routes as options and the state's route recommended. The first reply names the route and the two facts that chose it ("Built first: 14 files uncommitted on a branch 3 ahead of main, no plan folder"), so a wrong route costs one word to correct, and in its next sentence names any state fact that did not choose the route but changes the next move (an open PR with a red check, a checkout past the stale threshold, a plan folder at Ready), with the playbook that fact points to.
...
Leaf skills the playbooks call: `product-builder-setup`, `-how`, `-why`, `-research`, `-prototype`, `-personas`, `-pm`, `-techlead`, `-review`, `-verify`, `-resume`, `-reflect`, `-plain` (restate the last reply in plain words). A step that names one in bold invokes it with the Skill tool by that name (`product-builder-research`); it is never opened as a file with Read, because the harness substitutes its paths and registers its hooks only on invocation, and a step that reproduces a skill's work inline has skipped the skill. Templates, the Definition of Ready and Done, and the interaction examples: `${CLAUDE_SKILL_DIR}/references/`.

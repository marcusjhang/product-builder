...
3. Synthesise yourself, in the session, over the explorer returns (no explainer subagent): Overview; Key concepts; How it works; Where things live; Gotchas. Where two explorers disagree, read the file and settle it. Every claim carries file:line or a label (inferred, guess). Flag stale docs. When the profile names a judge, run its `anchor` gate over every anchored claim the explorers returned through `${CLAUDE_SKILL_DIR}/../product-builder/scripts/judge` (`--batch`; state: the claim, the anchor, ten lines from `git show <sha>:<path> | sed -n` around it); `refuted` is dropped and named in the reply, `unclear` you read yourself, `holds` stays yours to own; in `shadow` mode the verdicts are logged and nothing is dropped on the judge's word alone.

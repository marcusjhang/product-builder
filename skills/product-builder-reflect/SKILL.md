...
1. Read the plan folder (ledger, revision log, review rulings, qa report) and this session's transcript for friction: a question the user answered twice; a fact the profile lacked; a persona that was missing; a subagent that returned a dump; a step skipped without a reason; a Definition of Ready item red at hand-off; a fallback that fired; a budget exceeded.
...
4. Learnings: for each approved learning append a row to `.product-builder/learnings.md`: date, type (pattern, pitfall, preference, architecture, tool), confidence 1-10, files it refers to, one-line note. Only what saves time in a future session.

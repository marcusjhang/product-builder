...
5. One block, at most eight lines, grouped by bucket with seat and confidence, the proposed edit attached. Apply the reversible edits to `product.md` (stories, states, copy, assumptions, instrumentation, non-goals, deferred) and list them. Rulings that cut scope or change a product call are asked with one multi-select and the reply ends there; never write "waiting on you" and continue. Ledger rows per the mode's Subagents rule: one row per finding, every bucket, counts equal to the block, written before the reply.
